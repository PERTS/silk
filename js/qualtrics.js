...
/* createjs is defined lower in this file, after the perts module */
...
// CreateJS doesn't support certain environments at all, so don't even load
// the library in those cases.
if(!perts.is.IE8 && !perts.is.WinXP) {
    if (typeof createjs === 'undefined') {
        // If the library doesn't exist yet, we have a fresh environment and we
        // should load it from the minified code.

        // MODIFIED from vanilla version 0.6.2. Chris added code to protect
        // against references to a null gainNode, which causes bugs when
        // calling createjs.Sound.removeAllSounds() in Chrome.
