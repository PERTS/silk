...
//
// Heavily refactored for JFE and Neptune in March 2017.
//
// COMPATIBLE WITH NEPTUNE ONLY.
//
...
/* Crockford's json2.js library for serializing to and parsing from JSON. */
/* Ensures the native implentation of the global JSON objects is present. */
...
if (typeof $j === 'undefined') {
    // MFK 2014-07-12: Adding jQuery as '$j' to prevent conflicts.
    // CAM 2014-07-23: This line has to be in the global scope, not in an
    // onLoad event, otherwise everything breaks. Has to do with the fact that
    // Qualtrics loads its own instance of prototype under the dollar sign.
...
function PERTS_MODULE() {
  ...
  var domain,
    ...
    audioArgs = {},  // set by playAudioFile()
  ...
  var initAudioLibraries = function () {
  ...
  p.playAudioFile = function (fileName, options) {
    ...
    audioArgs.options = options;

    if (!p.isMobile()) {
      // Even if playImmediately is false, non-mobile clients benefit from
      // loading the audio file early. Mobile clients can't do this until within
      // a user-initiated event.
      initAudioLibraries().then(_playAudioFile);
    }
  };

  var _playAudioFile = function () {
    var deferred = $j.Deferred();

    if (audioContextCreated) {
      // If this was already run (e.g. on load) don't run it again on click.
      debugText += "\n _playAudiofile() shortcutting b/c already run";
      deferred.resolve();
      return deferred.promise();
    }

    var fileName = audioArgs.fileName;
    var options = audioArgs.options;
    ...
    if (typeof createjs !== 'undefined') {
      debugText += "\n createjs defined, going for WebAudio";
      createjs.FlashAudioPlugin.swfPath = 'https://www.perts.net/static/';
      ...
      createjs.Sound.addEventListener('fileload', function handleLoad(event) {
        ...
        // The file is loaded, functions waiting on it may continue.
        deferred.resolve();
      });
      ...
      debugText += "\n createjs NOT defined, allowing audioOn() to fall back to embed";
      // We won't be waiting on SoundJS to load any file; setup is done.
      deferred.resolve();
    ...
    // Don't try to run this function again, even if it's called again. Note
    // that this is set back to false in pageTransition().
  ...
  p.audioOn = function () {
    // Mobile devices don't initialize an audio context immediately because
    // they're only allowed to do so within a user-generate event, like a
    // touch/gesture.
    debugText += "\n audioOn() waiting for initAudioLibraries() and _playAudioFile()...";
    initAudioLibraries().then(_playAudioFile).then(function () {
      debugText += "\n ...resuming";
      ...
      if (currentSoundInstance) {
        debugText += "\n audioOn() found currentSoundInstance, playing";
        currentSoundInstance.play();
      } else {
        debugText += "\n audioOn() DIDNT find currentSoundInstance, falling back on embed";
  ...
  p.pageTransition = function () {
    // Allow a different audio file to be set up on the next page, otherwise
    // _playAudioFile() would be a noop.
    debugText += "\n pageTransition() setting audioContextCreated to false";
    audioContextCreated = false;
